...
| # | Question | Verdict |
|---|---|---|
| S1 | Does OCCT convert from a file's *declared* unit? | **PASS** (doubly confirmed) |
...
**Optional follow-up:** OCCT does not expose the *original* declared unit, so an info-panel line like
...
fixtures are the right mitigation. **Possible upgrade:** jsdom *does* provide `Image`, so textured USD
